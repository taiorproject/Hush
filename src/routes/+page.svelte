<script lang="ts">
  ...
  const filteredMessages = derived([messages, searchQuery], ([$m, $q]) => {
    ...
    return $m.filter(
      (msg) =>
        ...
        msg.alias.toLowerCase().includes(term),
    );
  });
  ...
  onMount(() => {
    const stored = localStorage.getItem("hush-theme") as
      | "light"
      | "dark"
      | "system"
      | null;
  ...
  const send = async (event: SubmitEvent) => {
    ...
    try {
      await session.sendMessage(text, reinforcedVal);
      form.reset();
    } catch (err) {
      // CRITICAL: AORP routing failed - show error to user
      const errorMsg = err instanceof Error ? err.message : String(err);
      errorDetails = errorMsg;
      errorModalOpen = true;
      console.error("AORP routing failed:", err);
    }
  };
...
<div class="h-screen flex flex-col bg-app text-[var(--text)] overflow-hidden">
  {#if $wasmError}
    <div
      class="h-screen flex items-center justify-center p-6 bg-red-50 dark:bg-red-950"
    >
      <div
        class="max-w-2xl w-full bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 border-2 border-red-500"
      >
        <div class="flex items-center gap-4 mb-6">
          <div
            class="size-16 rounded-full bg-red-100 dark:bg-red-900 flex items-center justify-center flex-shrink-0"
          >
            <svg
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Hush requiere libtaior WASM para funcionar de forma segura
            </p>
          </div>
        </div>

        <div
          class="bg-red-50 dark:bg-red-950/50 rounded-lg p-4 mb-6 border border-red-200 dark:border-red-800"
        >
          <p
            class="text-sm font-mono text-red-800 dark:text-red-200 whitespace-pre-wrap"
          >
            {$wasmError}
          </p>
        </div>
        ...
        <div class="space-y-4 text-sm">
          <div>
            ...
            <p class="text-gray-700 dark:text-gray-300">
              Hush usa <strong>libtaior</strong> para enrutamiento anónimo con cifrado
              multicapa (AORP). Sin WASM, la aplicación NO puede garantizar privacidad
              ni anonimato.
            </p>
          </div>
          ...
          <div>
            ...
            <div
              class="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 font-mono text-xs space-y-2"
            >
              <div>
                <span class="text-green-600 dark:text-green-400">$</span> cd ../libtaior
              </div>
              <div>
                <span class="text-green-600 dark:text-green-400">$</span> ./build-wasm.sh
              </div>
              <div>
                <span class="text-green-600 dark:text-green-400">$</span> cd ../Hush
              </div>
              <div>
                <span class="text-green-600 dark:text-green-400">$</span> npm install
                ../libtaior/pkg
              </div>
              <div>
                <span class="text-green-600 dark:text-green-400">$</span> npm run
                dev
              </div>
            </div>
          </div>
          ...
          <div
            class="bg-yellow-50 dark:bg-yellow-950/30 rounded-lg p-4 border border-yellow-200 dark:border-yellow-800"
          >
            <p class="text-xs text-yellow-800 dark:text-yellow-200">
              <strong>Advertencia:</strong> NO uses Hush sin libtaior WASM. Sería
              completamente inseguro y expondría tu IP y metadatos.
            </p>
          </div>
        </div>
  ...
  {:else}
    ...
    <header
      class="flex-shrink-0 bg-[var(--surface)] border-b border-[var(--border)] px-3 py-2.5 md:px-4 md:py-3"
    >
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <div
            class="hidden md:flex items-center gap-2 bg-[var(--surface-muted)] border border-[var(--border)] rounded-lg px-3 py-1.5"
          >
            <svg
              class="w-4 h-4 muted"
              fill="none"
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <aside
        class="hidden md:flex flex-col w-80 bg-[var(--surface)] border-r border-[var(--border)]"
      >
        <!-- Nueva conversación -->
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          {:else}
            {#each $roomHistory as room}
              <button
                type="button"
                class="w-full flex items-center gap-3 px-4 py-3 hover:bg-[var(--surface-muted)] transition-colors border-b border-[var(--border)] {$roomKey ===
                room
              ...
              >
                <div
                  class="size-12 rounded-full bg-[var(--accent)]/10 flex items-center justify-center font-semibold text-[var(--accent)] flex-shrink-0"
                >
                  {room[0]}
                </div>
        ...
        <div
          class="p-3 border-t border-[var(--border)] bg-[var(--surface-muted)]"
        >
      ...
      <main class="flex-1 flex flex-col bg-[var(--bg)] overflow-hidden">
        ...
        {#if $joined}
          ...
          <div
            class="flex-shrink-0 bg-[var(--surface)] border-b border-[var(--border)] px-3 py-3 md:px-4"
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <button
                ...
                <div
                  class="size-10 rounded-full bg-[var(--accent)]/10 flex items-center justify-center font-semibold text-[var(--accent)]"
                >
                  {($roomKey || "R")[0]}
                </div>
                <div>
              ...
              <!-- Indicador AORP Shield -->
              <div
                class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--surface-muted)] border border-[var(--border)]"
              >
                {#if $connected}
                  <svg
                  ...
                  <div class="text-xs font-medium">Taior: Activo</div>
                {:else}
                  <svg
                  ...
                  <div class="text-xs font-medium text-[var(--muted)]">
                    Circuitos: Iniciando
                  </div>
                {/if}
              </div>
          ...
          <!-- Mensajes -->
          <div
            class="flex-1 overflow-y-auto px-3 py-3 space-y-2 md:px-4 md:py-4"
          >
            {#each $filteredMessages as message (message.id)}
              <div
                class={`flex ${message.senderId === $hushIdStore ? "justify-end" : "justify-start"}`}
              >
                <div
                  class={`flex gap-2 max-w-[75%] ${message.senderId === $hushIdStore ? "flex-row-reverse" : "flex-row"}`}
                >
                  {#if message.senderId !== $hushIdStore}
                    <div
                      class="size-8 rounded-full bg-[var(--surface-muted)] flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-1"
                    >
                  ...
                  <div class="flex flex-col gap-1">
                    {#if message.senderId !== $hushIdStore}
                      <div class="text-xs font-medium px-3">
                        {message.alias}
                      </div>
                    {/if}
                    <div
                      class={`rounded-2xl px-4 py-2 ${message.senderId === $hushIdStore ? "bubble-self rounded-tr-sm" : "bubble-other rounded-tl-sm"}`}
                    >
                      <p
                      ...
                      <div
                        class="flex items-center gap-2 mt-1 text-[11px] opacity-75"
                      >
                        <span
            ...
            {#if $filteredMessages.length === 0}
              <div class="flex items-center justify-center h-full">
                <div class="text-center">
                  <div
                    class="size-16 rounded-full bg-[var(--surface-muted)] flex items-center justify-center mx-auto mb-3"
                  >
                    <svg
                      class="w-8 h-8 muted"
                      fill="none"
          ...
          <!-- Input de mensaje -->
          <form
            class="flex-shrink-0 bg-[var(--surface)] border-t border-[var(--border)] px-3 py-3 md:px-4 safe-bottom"
            on:submit={send}
          >
            <div class="flex items-end gap-2">
              <div
                class="flex-1 bg-[var(--surface-muted)] rounded-2xl px-4 h-12 md:h-12 flex items-center focus-within:ring-2 focus-within:ring-[var(--accent)] transition-colors"
              >
                <input
                  name="message"
            ...
            {#if $reinforced}
              <div
                class="flex items-center gap-1.5 mt-2 text-xs text-green-600 dark:text-green-400"
              >
                <svg
                  class="w-3.5 h-3.5"
                  fill="currentColor"
        ...
        {:else if $roomHistory.length === 0}
          ...
          <div class="flex-1 flex items-center justify-center p-8">
            <div class="text-center max-w-md px-4">
              <div
                class="size-20 rounded-full bg-[var(--surface)] border-2 border-[var(--border)] flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  class="w-10 h-10 text-[var(--accent)]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <p class="text-sm muted mb-6">
                Mensajería privada y segura con cifrado P2P. Crea o únete a una
                sala para comenzar.
              </p>
    ...
    {#if $showSheet}
      ...
      >
        ...
        >
          ...
          <div class="space-y-3">
            <div class="space-y-2">
              <Label for="room-modal" class="text-sm font-medium"
                >Clave de sala</Label
              >
            ...
            <div class="space-y-2">
              <Label for="alias-modal" class="text-sm font-medium"
                >Tu nombre</Label
              >
              <input
                id="alias-modal"
            ...
            <div
              class="flex items-center gap-3 rounded-lg bg-[var(--surface-muted)] px-4 py-3 border border-[var(--border)]"
            >
              <Checkbox
                bind:checked={$reinforced}
                id="reinforced-modal"
                name="reinforced-modal"
              />
              <div class="flex-1">
                <Label for="reinforced-modal" class="text-sm font-medium"
                  >Privacidad reforzada</Label
                >
                <div class="text-xs muted mt-0.5">
                  Mayor protección, menor velocidad
                </div>
              </div>
              <Badge color={$reinforced ? "green" : "dark"} class="text-xs"
                >{$reinforced ? "On" : "Off"}</Badge
              >
            </div>

            <div class="space-y-2">
              <div class="text-xs uppercase tracking-[0.2em] muted">
                Historial
              </div>
              {#if $roomHistory.length === 0}
            ...
            <div
              class="rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] px-3 py-2 text-xs flex items-center gap-2"
            >
              {#if $connected}
                <span class="size-2 rounded-full bg-green-500"></span>
                <span class="text-green-600 dark:text-green-400 font-medium"
          ...
          <div class="flex gap-3 pt-2">
            <Button
              ...
              class="flex-1"
              color="light"
              on:click={closeSheet}>Cancelar</Button
            >
            <Button
              type="button"
              class="flex-1"
    ...
    {#if $showProfile}
      <div
        class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center px-4 z-50"
        role="dialog"
        aria-modal="true"
      >
        <div
          class="w-full max-w-2xl bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 space-y-5 shadow-xl max-h-[90vh] overflow-y-auto"
        >
          <div class="flex items-center justify-between">
            <div>
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div
              class="space-y-3 rounded-xl bg-[var(--surface-muted)] border border-[var(--border)] p-3"
            >
              <div class="text-xs uppercase tracking-[0.2em] muted">
                ID de cuenta
              </div>
              ...
              <div class="flex gap-2">
                <Button color="light" on:click={() => copyText($hushIdStore)}
                  >Copiar</Button
                >
                <Button
                  color="blue"
              ...
              <div class="space-y-1 text-xs muted">
                ...
                <div
                  class="rounded-lg border border-[var(--border)] p-2 bg-[var(--surface)] text-[var(--text)]"
                >
                  Hola! Estoy usando Hush para chatear con total privacidad y
                  seguridad. Chatea conmigo! Mi ID de cuenta es {$hushIdStore}
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div class="text-xs muted">
                Guárdala para reinstalar en otro dispositivo.
              </div>
              <Button color="light" on:click={() => copyText($recoveryKey)}
                >Copiar clave</Button
              >
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div
              class="space-y-2 rounded-xl bg-[var(--surface-muted)] border border-[var(--border)] p-3"
            >
              <div class="text-xs uppercase tracking-[0.2em] muted">QR</div>
              <div class="text-xs muted">
                Escanea para agregar contacto. (Placeholder)
              </div>
              <div
                class="aspect-square rounded-lg border border-[var(--border)] bg-[var(--surface)] flex items-center justify-center text-xs muted"
              >
                QR pronto
              </div>
            </div>
            ...
            >
              ...
              <p class="text-sm muted">
                Hush oculta tu IP usando <strong>AORP</strong>.
                El tráfico viaja cifrado en capas por múltiples nodos.
              </p>
              <div class="space-y-1 text-xs">
                <div>• Cifrado: AES-GCM multicapa.</div>
                <div>• Circuitos: Rotación dinámica.</div>
                <div>• Privacidad: Tu IP está oculta.</div>
              </div>
              <Button
                color="light"
                on:click={() =>
                  shareText(
                    "Ruta Taior: tu IP se oculta vía libtaior con WebRTC P2P, sin servidores de datos.",
          ...
          <div
            class="space-y-2 rounded-xl bg-[var(--surface-muted)] border border-[var(--border)] p-3"
          >
            <div class="text-xs uppercase tracking-[0.2em] muted">Red AORP</div>
            <p class="text-sm muted">
              Conectado a la red de enrutamiento anónimo. Tus mensajes están
              protegidos por criptografía de grado militar.
            </p>
          </div>
        </div>
      </div>
  ...
  <!-- Modal de Error de Privacidad -->
  <Modal bind:open={errorModalOpen} size="sm" class="w-full">
    <div class="text-center p-4">
      <div
        class="size-16 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center mx-auto mb-4"
      >
        <svg
      ...
      <p class="mb-4 text-gray-500 dark:text-gray-400">
        Tu mensaje <strong>NO fue enviado</strong> para proteger tu anonimato.
      </p>

      <div
        class="text-left bg-red-50 dark:bg-red-900/20 p-3 rounded-lg mb-6 border border-red-100 dark:border-red-900/50"
      >
        <p class="text-xs font-mono text-red-700 dark:text-red-400 break-words">
          {errorDetails}
        </p>
      </div>

      <div class="flex justify-center gap-3">
        <Button color="red" on:click={() => (errorModalOpen = false)}
          >Entendido</Button
        >
      </div>
    </div>
